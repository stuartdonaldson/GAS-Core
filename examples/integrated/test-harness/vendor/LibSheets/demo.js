...
/**
 * vendor/LibSheets/demo.js — flat vendored copy of
 * libs/LibSheets/harness/demo.js for the shared aggregator
 * (examples/integrated/test-harness/Code.js). Kept in sync verbatim with the
 * canonical file (same vendoring discipline as production libs, per
 * scripts/check-lib-drift.sh) EXCEPT for the registration tail at the bottom
 * of this file, which is vendor-only boilerplate (see note above
 * `HARNESS_DEMOS_` below) — it exists only because Apps Script's V8 runtime
 * loads every file in a clasp project into ONE shared global scope with no
 * `require()`/module resolution, so multiple vendored demo.js files cannot
 * each declare a bare top-level `const demos` (collision) or be loaded via
 * `require()` (undefined in GAS V8). The canonical libs/LibSheets/harness/
 * demo.js is unaffected; only this flat copy's tail differs, by design.
 *
...
if (typeof module !== 'undefined' && module.exports) {
  // Node test-sandbox path (used by examples/integrated/test-harness's own
  // Node-side verification and any future Tier-1-style check of the demo
  // registration itself) — unchanged from the canonical file's I4 contract.
  module.exports = { demos };
} else {
  // Real Apps Script V8 runtime path: no `module`, no `require()`, and every
  // file in the clasp project shares ONE global scope, so this vendored
  // copy registers itself on a namespaced global instead of relying on a
  // bare top-level `const demos` (which would collide with every other
  // vendored library's same-named `demos` declaration once multiple
  // libraries are vendored side by side). HARNESS_DEMOS_ is declared with
  // `var` (not `const`/`let`) specifically because `var` redeclaration
  // across files is tolerated by V8 GAS, unlike `const`/`let`.
  // eslint-disable-next-line no-undef, no-var
  var HARNESS_DEMOS_ = (typeof HARNESS_DEMOS_ !== 'undefined') ? HARNESS_DEMOS_ : {};
  HARNESS_DEMOS_.LibSheets = demos;
}
