 ...
 * (examples/integrated/test-harness/Code.js). Generated by
 * scripts/copy-demo-files.sh — DO NOT HAND-EDIT. Kept in sync verbatim
 * with the canonical file (same vendoring discipline as production libs,
 * per scripts/check-lib-drift.sh) EXCEPT for the registration tail at the
 * bottom of this file, which is vendor-only boilerplate appended by this
 * script — it exists only because Apps Script's V8 runtime loads every
 * file in a clasp project into ONE shared global scope with no
 ...
 * `require()` (undefined in GAS V8). The canonical libs/LibSheets/harness/demo.js
 * is unaffected; only this flat copy's tail differs, by design.
 *
