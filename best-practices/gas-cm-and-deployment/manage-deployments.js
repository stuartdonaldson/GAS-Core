 ...
 *   npm run deploy:test          # stamp revision + redeploy TEST
 *   npm run deploy:prod          # stamp revision + redeploy PROD
 *   npm run manage-deployments   # interactive menu (list/archive/deploy)
 *
 * Direct invocation (flags only, no npm):
 *   node manage-deployments.js --deploy-test     # skips update-revision — warns
 *   node manage-deployments.js --deploy-prod     # skips update-revision — warns
 ...
 *   TEST  → deployment with description containing "TEST-WEB-APP"
 *   PROD  → deployment with description containing "PROD-WEB-APP"
 *
 * IDs are discovered at runtime by substring-searching `clasp deployments`
 * output for the anchor strings above. There is no config file of deployment
 * IDs. Each deploy sets the description to include the anchor, the version,
 * and the revision timestamp from src/version.html — making deployment history
 * auditable directly from the deployments list. Example:
 *   "PROD-WEB-APP v1.6.2 (Rev. May 7, 2026 07:26)"
 *
 * After deployment, metadata is written to .deploy-metadata.json (gitignored)
 * for commit-deploy-stamp.js to use in the git commit message.
 *
 * ─────────────────────────────────────────────────────────────────────────
 * ONE-TIME SETUP
 * ─────────────────────────────────────────────────────────────────────────
 ...
 *      created once in the Apps Script editor as a Web App, with a
 *      description containing "TEST-WEB-APP" or "PROD-WEB-APP" (the anchor
 *      is a substring match — subsequent deploys will append version info).
...
function getVersionFromBuildInfo() {
  const versionFile = path.join(SRC_DIR, 'version.html');
  const content = fs.readFileSync(versionFile, 'utf8');
  const match = content.match(/version:\s*"([^"]+)"/);
  return match ? match[1] : 'unknown'; // e.g. "v1.6.2 (Rev. May 7, 2026 07:26)"
}
...
async function main() {
  try {
    if (!process.env.npm_lifecycle_event) {
      ...
      console.warn('   For deployments use: npm run deploy:test  or  npm run deploy:prod');
      console.warn('');
    }
...
async function deployToTarget(target, deployments, nonInteractive) {
  ...
  console.log(`   clasp deploy -i ${match.deploymentId} -d "${description}"`);
  execSync(`clasp deploy -i ${match.deploymentId} -d "${description}"`, { stdio: 'inherit' });

  // Capture new deployment revision for commit-deploy-stamp.js
  ...
  if (updated) {
    ...
    fs.writeFileSync(path.join(__dirname, '.deploy-metadata.json'), JSON.stringify(metadata, null, 2));
  }
