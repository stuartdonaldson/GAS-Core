...
**A successful `assertDeployedVersion` is not proof the whole fleet has turned over.** Google Apps
Script rolls a new version out across its edge over tens of seconds, and during that window
different requests reach different code. This was rediscovered independently in three consecutive
PracticeMix migration stages before anyone wrote it down; it costs a diagnostic budget every time,
because the symptom — one action answering with new behaviour while another answers with old — reads
exactly like a bug in the change that just shipped.

The measurements, all against live TEST:

| What changed | Observed convergence | What was seen mid-window |
|---|---|---|
| Server-side **code** (P1R) | ~1 min / 3 retries | `cmd=version` returned the *old* version while one action already ran new code and another still ran old |
| **`appsscript.json`** manifest change (P2) | ~90 s | same shape, slower — a manifest edit is the slow case |
| First **GitHub Pages publish** to a new directory (P4) | ~35 s | `version.json` 404s, then serves |

Three consequences, all of them now defaults rather than advice:

1. **Settle on N consecutive reads, not the first one.** `assertDeployedVersion` requires
   `settleReads` agreeing reads (default **2**, configurable; `settleReads: 1` restores the old
   behaviour), spaced by the poll interval, and a disagreeing read in between resets the count. One
   agreeing read only proves *an* edge has turned over.
2. **Budget minutes, not the default minute.** `assertDeployedVersion`'s `timeoutSec` still defaults
   to 60, which is under the measured convergence for a manifest change — raise it via
   `verifyOptions` on any project that edits `appsscript.json` as part of a deploy.
   `gas-static`'s `assertPublishedBuild` defaults to **300**, matching what `deployHooks()` has
   always passed and the CDN numbers above.
3. **Do not treat the window as an error, and do not paper over it either.** A retry loop around a
   post-deploy smoke test is right; a `sleep` picked by feel is not, and neither is a suite that
   asserts on the first response after a deploy. Where a user can be inside the window, the static
   page's update banner is the backstop — see
   [`../gas-static-frontend/README.md`](../gas-static-frontend/README.md) §The static page interface
