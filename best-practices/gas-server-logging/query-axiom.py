#!/usr/bin/env python3
"""
query-axiom.py — Query an Axiom dataset without re-deriving the APL request shape
and auth every time.

Reads axiomDataset + axiomQueryToken from local.settings.json (gitignored — copy
from local.settings.example.json). Never prints the token.

Usage:
    python query-axiom.py [--limit N] [--since DURATION] [--name SUBSTRING]
...
Examples:
    ...
    python query-axiom.py --raw /tmp/axiom_dump.json   # also dump full JSON for offline analysis
...
_SETTINGS_PATH = pathlib.Path(__file__).parent / "local.settings.json"
...
def _load_settings() -> dict:
    if not _SETTINGS_PATH.exists():
        raise FileNotFoundError(
            f"{_SETTINGS_PATH} not found. Copy local.settings.example.json and fill in your Axiom values."
        )
...
def query(dataset: str, token: str, *, limit: int, since: timedelta,
          name: str | None, where: str | None) -> dict:
...
def _print_table(matches: list[dict]) -> None:
    for m in matches:
        ...
        print(f"{m['_time']}  {name:<32} {detail}")
...
def main() -> int:
    ...
    result = query(
        ...
        name=args.name, where=args.where,
    )
