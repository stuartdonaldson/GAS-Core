# Best Practice: Google Workspace Add-on Setup, Testing & Distribution

## Overview

This pattern covers setting up, testing, and architecting Google Workspace Add-ons — the
new-style `addOns` manifest (not the deprecated `addon` Editor Add-on key). It's split into two
concerns that are easy to conflate but diagnosed very differently when something isn't showing
up: **where a given add-on surface is actually supposed to render** (side panel vs. Extensions
menu vs. legacy bound-script menu), and **GCP/Marketplace SDK plumbing** (required APIs, OAuth
consent, deployment-version binding) that has to be right before any of those surfaces can work
at all.

The most expensive failure mode this pattern guards against: a surface not appearing looks
identical whether the cause is (a) code/manifest, (b) looking in the wrong UI location for that
surface type, or (c) a GCP-side deployment-version pointer gone stale — and (b) and (c) are both
non-code causes that a code bisect will never find.

**Source:** GActionSheet (`sdonaldson@northlakeuu.org`, `stuart.donaldson@gmail.com`), original
setup 2026-05-22, Extensions-menu/Marketplace-SDK-version findings added 2026-08-12. Full history
in that project's `knowledge-base/references/workspace-addon-setup.md`.
...
Add a `.nojekyll` file at the repo root so Jekyll does not interfere with asset paths.
...
> **Not verified in practice** — documented by Google but could not be made to work. Treat as a fallback to investigate, not a reliable path.
...
3. Because the app is private, Google drops manual evaluation. Placeholder values are accepted:
   - **Legal links:** Paste your org homepage into the Privacy Policy, Terms of Service, and Support URL fields — the validator accepts it.
